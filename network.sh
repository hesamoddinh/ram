...
cp -f "$TEMPLATES_DOCKER_COMPOSE_FOLDER/docker-compose-couch.yaml" "$GENERATED_DOCKER_COMPOSE_FOLDER"
...
function getCouchDbPortForOrg () {
    peer_port=$(( $1 ))
    local couch_db_port=${peer_port}+10000
    echo $(( couch_db_port ))
}
...
function generatePeerArtifacts() {
    ...
    peer0_couch_db_port=$(getCouchDbPortForOrg peer0_port)
    peer1_couch_db_port=$(getCouchDbPortForOrg peer1_port)
    ...
    # docker-compose yaml
    sed -e "s/PEER0_COUCHDB_PORT:5984/\"$peer0_couch_db_port:5984\"/g" -e "s/PEER1_COUCHDB_PORT:5984/\"$peer1_couch_db_port:5984\"/g" -e "s/PEER0_COUCHDB_PORT/$peer0_couch_db_port/g" -e "s/PEER1_COUCHDB_PORT/$peer1_couch_db_port/g" -e "s/PEER_EXTRA_HOSTS/$peer_extra_hosts/g" -e "s/CLI_EXTRA_HOSTS/$cli_extra_hosts/g" -e "s/API_EXTRA_HOSTS/$api_extra_hosts/g" -e "s/DOMAIN/$DOMAIN/g" -e "s/\([^ ]\)ORG/\1$org/g" -e "s/API_PORT/$api_port/g" -e "s/WWW_PORT/$www_port/g" -e "s/CA_PORT/$ca_port/g" -e "s/PEER0_PORT/$peer0_port/g" -e "s/PEER0_EVENT_PORT/$peer0_event_port/g" -e "s/PEER1_PORT/$peer1_port/g" -e "s/PEER1_EVENT_PORT/$peer1_event_port/g" ${compose_template} | awk '{gsub(/\[newline\]/, "\n")}1' > ${f}
#    sed -e "s/PEER0_COUCHDB_PORT/$peer0_couch_db_port/g" -e "s/PEER1_COUCHDB_PORT/$peer1_couch_db_port/g" ${compose_template} | awk '{gsub(/\[newline\]/, "\n")}1' > ${f}
...
#  do
    installChaincode ${org} ${CHAINCODE_COMMON_NAME} "1.0"
#  done
}
...
function devInstall () {
  docker-compose -f ${COMPOSE_FILE_DEV} run cli bash -c "peer chaincode install -p mycc -n mycc -v 0"
}
...
if [ "${MODE}" == "up" -a "${ORG}" == "" ]; then
  ...
  do
    TIMEOUT=${CLI_TIMEOUT} COMPOSE_HTTP_TIMEOUT=${CLI_TIMEOUT} dockerComposeUp ${org}
  done
...
#  joinWarmUp ${ORG3} "${ORG2}-${ORG3}" ${CHAINCODE_BILATERAL_NAME}
  ...
  clean
  removeArtifactsdoc     ps


  generatePeerArtifacts ${ORG1} 4000 9091 7054 7051 7053 7056 7058
  generatePeerArtifacts ${ORG2} 4001 9092 8054 8051 8053 8056 8058
  generatePeerArtifacts ${ORG3} 4002 9093 9054 9051 9053 9056 9058
