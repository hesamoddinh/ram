...
function UserService($log, $rootScope, ApiService, localStorageService) {
  ...
  UserService._setRefreshTimer = function() {
    ...
    console.warn('AuthService: refresh token in %s ms', refreshTimeout);
  ...
  UserService.canAccess = function(state){
    ...
    // console.log('AuthService.canAccess:', isAllowed, state.name);
    return isAllowed; //&& AuthService.isAuthorized();
  };
