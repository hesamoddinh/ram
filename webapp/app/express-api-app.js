...
// /////////////////////////////////////////////////////////////////////////////
var myHelper = require('../lib-fabric/my-helper.js')

// Register User
adminPartyApp.post('/register', function (req, res) {
	...
	res.promise(
		myHelper.registerUser(username, org, 'aMSP')
			.then((user) => {
				return {
					success: true,
...
app.post('/channels/:channelName/chaincodes/:chaincodeName', function (req, res) {
	...
	var channelName = req.params.channelName
	var peersId = req.body.peers.split(',') || ''
	var peers = peersId.map(getPeerHostByCompositeID)
	...
	res.promise(
		invoke.invokeChaincode(peers, channelName, chaincodeName, fcn, args, req.username, ORG)
			.then(function (response) {
				return {transaction: response.txId, result: response.result}
			})
			.catch(err => {
				...
				if (err) {
					...
					if (err.msg) {
						msg = err.msg.split(',')[1].split(':')[1].slice(0, -1).trim()
					}
				}
				return {ok: false, msg: msg, errorCode: errCode}
			})
	)
})
...
app.get('/channels/:channelName/chaincodes/:chaincodeName', function (req, res) {
	...
	res.promise(
		query.queryChaincode(peerInfo.peer, channelName, chaincodeName, args, fcn, req.username, ORG)
			.catch(err => {
				return {ok: false, msg: err.message, errorCode: err.code}
			})
	)
})
