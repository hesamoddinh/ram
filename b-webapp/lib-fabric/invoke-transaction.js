  ...
  return helper.getChannelForOrg(channelID, username, org)
    .then(_channel=>{
      ...
      var client = channel.getClient();
			...
			return channel.sendTransactionProposal(request,5);
		})

		.then((results) => {
			if (results[0][0].response) {
                result = results[0][0].response.payload.toString('utf-8')
			...
			if (lastError) {
			    let err = {
			        msg: proposalResponses[0].details,
                    code: proposalResponses[0].code
                }
			    return Promise.reject(err || 'Error!')
				// throw new Error(proposalResponses[0].details||'Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
			}
		...
		.then((response) => {
			if (response.status === 'SUCCESS') {
				...
				return {txId: tx_id.getTransactionID(), result: result}
			} else {
