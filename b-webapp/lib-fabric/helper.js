...
// var FabricCAClient = require('fabric-ca-client/lib/FabricCAClientImpl');
...
function _newClient(username, orgID){
    console.log("_newClient .... ",username, orgID)
  if(!ORGS[orgID]){
 ...
 * @returns {Promise<User>}
 */
function createUser(admin, org, username, password, role, affiliation, maxEnrollments, attrs){
    ...
    /*getClientForOrg(admin,org).then(client => {
        let caClient = new FabricCAClient({protocol: 'http', hostname: 'localhost', port: getCaServerPort(org)})
        caClient.register(username, password, role, affiliation, maxEnrollments, attrs, client)
            .then(secret => {
    ...
    return getClientForOrg(admin, org)
        .then(client=>{
            client.createUser()
        });*/
}
